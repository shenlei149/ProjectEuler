...
std::string S932::GetAnswer()
{
	...
	// Max square is (10^8)^2 = 10^16. Fits in int64_t.
	...
	int64_t sum = 0;
	for (int64_t i = 4; i <= RootUpper; i++) // <= or < ? Code said <
	{
...
bool S932::Is2005Number(int64_t root, int64_t square)
{
	...
	for (int k = 1; k < len; ++k)
	{
		...
		int64_t power = POW[len - k];
		int64_t a = square / power;
		int64_t b = square % power;
