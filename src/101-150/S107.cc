...
std::string S107::GetAnswer()
{
	// Use large dataset
	auto matrix = InitMatrix(large);

	// Lazy Prim's algorithm
	std::vector<bool> marked(matrix.size(), false);
	// Min-heap for edges
	...
	while (!pq.empty())
	{
		...
		if (!marked[v])
		{
			Visit(matrix, v, marked, pq);
		}

		if (!marked[w])
		{
			Visit(matrix, w, marked, pq);
		}
	}

	return std::to_string((totalWeight / 2 - mst));
}

void S107::Visit(const std::vector<std::vector<int>> &matrix,
				 int v,
				 ...
				 std::priority_queue<Edge, std::vector<Edge>, std::greater<Edge>> &pq)
{
	...
	{
		if (!marked[i] && matrix[v][i] != INT_MAX)
		{
			pq.push(Edge(v, i, matrix[v][i]));
		}
	}
}

std::vector<std::vector<int>> S107::InitMatrix(const std::vector<std::string> &graph)
{
...
std::vector<std::string> S107::little = { "-,16,12,21,-,-,-",	 "16,-,-,17,20,-,-", "12,-,-,28,-,31,-",
										  "21,17,28,-,18,19,23", "-,20,-,18,-,-,11", "-,-,31,19,-,-,27",
										  "-,-,-,23,11,27,-" };

std::vector<std::string> S107::large = {
