...
{
    public class S107 : ISolution
    {
        private int totalWeight = 0;

        public string GetAnswer()
        {
            var matrix = InitMatrix(large);

            // Lazy Prim's algorithm
            bool[] marked = new bool[matrix.Count];
            ...
            return (totalWeight / 2 - mst).ToString();
        }

        private static void Visit(List<List<int>> matrix, int v, bool[] marked, PriorityQueue<Edge, int> pq)
        {
            ...
            {
                if (!marked[i])
                { pq.Enqueue(new Edge(v, i, matrix[v][i]), matrix[v][i]); }
            }
        }

        private List<List<int>> InitMatrix(string[] graph)
        {
            ...
            foreach (var line in graph)
            {
                ...
                foreach (var weightStr in items)
                {
                    if (weightStr == "-")
                    { edges.Add(int.MaxValue); }
                    else
                    {
        ...
        private static string[] little = new string[]{
            "-,16,12,21,-,-,-",
            "16,-,-,17,20,-,-",
            "12,-,-,28,-,31,-",
            "21,17,28,-,18,19,23",
